...
export default function Onboarding() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col pt-12 md:pt-24 px-4 items-center">
      <div className="w-full max-w-xl">
        <div className="flex gap-2 mb-8">
          {[1, 2, 3].map(s => (
            <div key={s} className={`h-2 flex-1 rounded ${s <= step ? 'bg-primary' : 'bg-primary/20'}`} />
          ))}
        </div>

        <div className="bg-card border border-border p-6 md:p-10 rounded-lg shadow-2xl">
          {step === 1 && (
            <div className="animate-in slide-in-from-right-4 fade-in duration-300">
              <p className="text-primary font-bold uppercase tracking-widest text-xs mb-2">Step 1 of 3</p>
              <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tight mb-8 text-foreground">Operation Basics</h2>
              
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-bold uppercase tracking-widest text-muted-foreground mb-3">Operation Name</label>
                  <input 
                    type="text" 
                    value={formData.operationName}
                    onChange={e => setFormData({...formData, operationName: e.target.value})}
                    placeholder="e.g. Valley Farms"
                    className="w-full bg-background border border-border rounded px-4 py-4 text-foreground text-lg focus:outline-none focus:border-primary"
                  />
                </div>
                
                <div>
                  <label className="block text-sm font-bold uppercase tracking-widest text-muted-foreground mb-3">Primary Crops</label>
          ...
          {step === 2 && (
            <div className="animate-in slide-in-from-right-4 fade-in duration-300">
              ...
              <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tight mb-8 text-foreground">First Block</h2>
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      {seasons.map(s => (
                        <label key={s} className={`p-3 border rounded cursor-pointer flex items-center gap-3 ${formData.seasonGroup === s ? 'border-primary bg-primary/5' : 'border-border'}`}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      {yields.map(y => (
                        <label key={y} className={`p-3 border rounded cursor-pointer flex items-center gap-3 ${formData.yieldTarget === y ? 'border-primary bg-primary/5' : 'border-border'}`}>
          ...
          {step === 3 && (
            <div className="animate-in slide-in-from-right-4 fade-in duration-300">
              ...
              <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tight mb-8 text-foreground">Plan Preview</h2>
              
              <div className="bg-background p-6 rounded-lg border border-border mb-8 space-y-6">
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Target Yield</h4>
                  <p className="text-2xl font-black text-foreground">{formData.yieldTarget} Bins / Acre</p>
                </div>
                
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border">
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Fertilizer Guide</h4>
                    <p className="text-xl font-bold text-primary">250-300 <span className="text-sm font-medium text-muted-foreground">lbs/ac</span></p>
                  </div>
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Water Target</h4>
                    <p className="text-xl font-bold text-blue-400">3.0 <span className="text-sm font-medium text-muted-foreground">ac-ft</span></p>
                  </div>
                </div>
              </div>
