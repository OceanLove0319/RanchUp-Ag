...
import { useRanches, useBlocks, useFieldLogs, useChemicalApps, useRecommendations } from "@/hooks/useData";
...
export default function Home() {
  ...
  const pcaStats = useMemo(() => {
    ...
    const ranchesWithStats = allRanches.map(ranch => {
      ...
      return { ...ranch, openRecs, pendingFollowThrough, recordIssues, isClean, overallStatus, ranchBlocks };
    });

    return { ranchesCount: allRanches.length, needsAttentionRanches, pendingFollowThroughTotal, recordsBlockingTotal, ranches: ranchesWithStats };
    ...
    return (
      <div className="animate-in fade-in duration-500 max-w-4xl mx-auto pb-20">
        <header className="mb-6 flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter text-foreground">PCA Command Center</h1>
            <p className="text-muted-foreground font-medium text-sm flex items-center gap-2">
              {user?.name} • {user?.org}
            </p>
          </div>
          <Link href="/app/packets/season">
            <button className="bg-card border border-border text-foreground px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest hover:border-primary/50 transition-colors">Export</button>
          </Link>
        </header>
        ...
        <div className="space-y-4">
          ...
          {pcaStats.ranches.map(ranch => (
            <button key={ranch.id} onClick={() => handleOpenRanch(ranch.id)}
  ...
  if (isPCA && pcaView === 'RANCH' && activeRanch) {
    ...
    activeRanchRecs.filter(r => r.status === 'PENDING').forEach(rec => {
      ...
      priorities.push({ id: `rec-${rec.id}`, type: 'follow-through', title: `${block?.name}: Rec pending`, desc: `Recommendation for ${rec.product} is waiting on grower.`, action: 'Review', link: `/app/blocks/${rec.blockId}`, color: 'text-purple-400', bg: 'bg-purple-400/10' });
    });
    activeRanchApps.filter(a => a.costStatus === 'UNIT_MISMATCH' || !a.estimatedCost).forEach(app => {
      ...
      priorities.push({ id: `app-${app.id}`, type: 'record-issue', title: `${block?.name}: Record Issue`, desc: `Application record for ${app.chemicalName} is missing data.`, action: 'Fix', link: `/app/reports/variance`, color: 'text-red-400', bg: 'bg-red-400/10' });
    });

    return (
      <div className="animate-in fade-in duration-500 max-w-4xl mx-auto pb-20">
        <button onClick={() => setPcaView('ALL')} className="flex items-center text-xs font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground mb-6 transition-colors">
          <ArrowLeft className="w-4 h-4 mr-2" /> All Ranches
        </button>
        <header className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter text-foreground">{activeRanch.name}</h1>
            {ranchStat?.isClean && <Badge className="bg-green-500/20 text-green-400 border-green-500/30 text-xs py-1"><CheckCircle2 className="w-3 h-3 mr-1" /> Packet Ready</Badge>}
          </div>
          ...
          <p className="mt-4 text-lg font-bold text-foreground">
            {ranchStat?.isClean ? "This ranch is clean and packet-ready." : `${ranchStat?.recordIssues} record issues are blocking a clean export.`}
          </p>
        </header>

        {priorities.length > 0 && (
          <div className="mb-10">
            <h2 className="text-sm font-black uppercase tracking-widest text-muted-foreground mb-3 flex items-center gap-2"><AlertCircle className="w-4 h-4" /> What needs attention</h2>
              ...
              {priorities.slice(0, 5).map(priority => (
                <div key={priority.id} className="bg-card border border-border p-4 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm hover:border-primary/30 transition-colors">
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-md ${priority.bg} ${priority.color} shrink-0 mt-1`}><AlertCircle className="w-4 h-4" /></div>
                    <div>
                  ...
                  <Link href={priority.link} className="w-full sm:w-auto">
                    <button className="w-full sm:w-auto bg-background border border-border px-6 py-3 rounded-lg text-xs font-black uppercase tracking-widest hover:border-primary hover:text-primary transition-colors">{priority.action}</button>
                  </Link>
                </div>
              ))}
            </div>
              ...
              return (
                <div key={block.id} className="bg-card border border-border p-4 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-primary/50 transition-colors group">
                  ...
                  <div className="flex flex-wrap md:flex-nowrap items-center gap-4 text-xs font-medium w-full md:w-auto flex-1 md:justify-end">
                    <div className="flex flex-col"><span className="text-muted-foreground/60 uppercase tracking-widest text-[10px] mb-0.5">Latest Rec</span><span className={latestRec ? "text-blue-400" : "text-muted-foreground"}>{latestRec?.status || 'None'}</span></div>
                    <div className="flex flex-col ml-4"><span className="text-muted-foreground/60 uppercase tracking-widest text-[10px] mb-0.5">Record Status</span><span className={flagCount > 0 ? "text-red-400 font-bold" : "text-green-400"}>{flagCount > 0 ? `${flagCount} Flags` : 'Clean'}</span></div>
                  </div>
                  <div className="w-full md:w-auto mt-2 md:mt-0">
                    <Link href={`/app/blocks/${block.id}`}>
                      <button className="w-full md:w-auto bg-background border border-border px-6 py-3 rounded-lg text-xs font-black uppercase tracking-widest group-hover:border-primary group-hover:text-primary transition-colors text-center">Open</button>
                    </Link>
                  </div>
                </div>
              );
  ...
  // GROWER VIEW
  if (!isPCA) {
    const pendingRecsGrower = allRecommendations.filter(r => r.status === 'SENT' || r.status === 'PENDING');
    const missingInfoAppsGrower = allApps.filter(a => a.costStatus === 'UNIT_MISMATCH' || !a.estimatedCost);

    return (
      <div className="animate-in fade-in duration-500 max-w-4xl mx-auto flex flex-col min-h-[calc(100vh-100px)] pb-12">
        <header className="mb-6 flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter text-foreground">Ranch Dashboard</h1>
            <p className="text-muted-foreground font-medium text-sm flex items-center gap-2">{activeRanch?.name || 'Select a ranch'}</p>
          </div>
        ...
        <div className="mb-8">
          <h2 className="text-sm font-black uppercase tracking-widest text-muted-foreground mb-3 flex items-center gap-2"><AlertCircle className="w-4 h-4" /> What needs attention today</h2>
          <div className="grid grid-cols-1 gap-3">
            {pendingRecsGrower.slice(0, 2).map(rec => {
              ...
              return (
                <Link key={`rec-${rec.id}`} href={`/app/blocks/${rec.blockId}`} className="bg-card border border-border p-4 rounded-xl flex items-start gap-4 shadow-sm hover:border-primary/50 transition-colors group">
                  <div className="p-2 rounded-md bg-purple-500/10 text-purple-400 shrink-0 mt-0.5"><ClipboardList className="w-5 h-5" /></div>
                  <div className="flex-1">
            ...
            {missingInfoAppsGrower.slice(0, 2).map(app => {
              ...
              return (
                <Link key={`app-${app.id}`} href="/app/reports/variance" className="bg-card border border-border p-4 rounded-xl flex items-start gap-4 shadow-sm hover:border-primary/50 transition-colors group">
                  <div className="p-2 rounded-md bg-red-500/10 text-red-400 shrink-0 mt-0.5"><AlertCircle className="w-5 h-5" /></div>
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
            ...
            {pendingRecsGrower.length === 0 && missingInfoAppsGrower.length === 0 && (
              <div className="bg-card border border-border border-dashed p-6 rounded-xl flex items-center gap-4 text-muted-foreground">
                <div className="p-2 rounded-md bg-green-500/10 text-green-400 shrink-0"><CheckCircle2 className="w-5 h-5" /></div>
                <div><h3 className="font-bold text-sm text-foreground">You're all caught up</h3><p className="text-xs">No pending recommendations or record issues.</p></div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mb-8">
          <div className="flex justify-between items-end mb-3">
            ...
            <Link href="/app/recommendations" className="text-[10px] font-bold text-primary hover:text-primary/80 uppercase tracking-widest">View All</Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {allRecommendations.slice(0, 4).map(rec => {
              const block = allBlocks.find(b => b.id === rec.blockId);
                ...
                <div key={`rec-card-${rec.id}`} className="bg-card border border-border p-5 rounded-xl flex flex-col justify-between shadow-sm hover:border-primary/30 transition-colors">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Badge variant="outline" className={`text-[10px] uppercase tracking-widest ${rec.status === 'APPLIED' ? 'text-green-400 border-green-400/30' : 'text-blue-400 border-blue-400/30'}`}>{rec.status}</Badge>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4 bg-background/50 p-3 rounded-lg border border-border/50">
                      <div><p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-0.5">Est. Cost/Ac</p><p className="text-sm font-bold text-foreground">${(rec.estimatedCostPerAcre || 0).toFixed(2)}</p></div>
                      <div><p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-0.5">Est. Block Total</p><p className="text-sm font-bold text-primary">${(rec.estimatedTotalCost || 0).toLocaleString()}</p></div>
                    </div>
                  </div>
                  <Link href={`/app/blocks/${rec.blockId}`}>
                    <button className="w-full bg-background border border-border py-2 rounded-lg text-xs font-black uppercase tracking-widest hover:border-primary hover:text-primary transition-colors">Open Recommendation</button>
                  </Link>
                </div>
              );
            })}
            {allRecommendations.length === 0 && (
              <div className="col-span-full py-8 text-center text-muted-foreground border border-border border-dashed rounded-xl">No active recommendations for this ranch.</div>
            )}
          </div>
        </div>

        <div className="mb-6">
          <h2 className="text-sm font-black uppercase tracking-widest text-muted-foreground mb-3 flex items-center gap-2"><DollarSign className="w-4 h-4" /> Cost Optimization Opportunities</h2>
  ...
  return (
    <div className="flex items-center justify-center h-64 text-muted-foreground text-sm font-medium uppercase tracking-widest">Loading...</div>
  );
}
